...
Evaluate a pretrained PPO agent on Pong and write a step-by-step CSV trace.
...
The CSV schema matches eval_pong_dqn.py so results from both agents can be
...
-------------------------------------------------------------------------------
    python eval_pong_ppo.py \
        --model_path runs/pong_ppo/ppo_pong.zip \
        --episodes 15 \
        --deterministic \
        --no_clip_reward \
        --log_path logs/ppo_pong_15ep.csv
...
With rendering:
    ...
    python eval_pong_ppo.py \
        ...
        --deterministic \
        --render \
        --log_path logs/ppo_pong_rendered.csv
...
- Unlike Breakout, Pong does not require a FIRE action to start play. The
  AutoFireWrapper is still included to be consistent with the Breakout scripts,
  but on_life_loss is set to False since Pong has no lives mechanic.

- Episodes are capped at max_episode_steps (default 10 000 decision steps) via
  a TimeLimit wrapper, consistent with all other eval scripts in this project.
"""
...
class AutoFireWrapper(gym.Wrapper):
    ...
    For Pong, on_life_loss defaults to False since Pong has no lives mechanic.
    The wrapper is still included here for consistency with the Breakout scripts.
...
def make_env(
    ...
    """Build and return a wrapped Pong environment for PPO evaluation.

    Uses AtariPreprocessing (gymnasium) to match the preprocessing applied
    during PPO training.
    """
    env = gym.make(
        env_id,
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate a pretrained PPO agent on Pong and log a CSV trace."
    )
    ...
    parser.add_argument("--log_path", type=str, default=r".\logs\ppo_pong_eval.csv",
    ...
    parser.add_argument("--max_episode_steps", type=int, default=10000,
                        help="Episode step cap.")
    parser.add_argument("--fire_presses", type=int, default=2,
                        help="Number of FIRE actions to send after reset.")
    ...
    # CSV schema is identical to eval_pong_dqn.py for direct comparability.
